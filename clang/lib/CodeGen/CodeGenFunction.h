...
public:
  ...
  // Track SSA versions of variables
  std::map<std::string, int> SSAVersions;
  std::map<std::string, std::string> OriginalVariable;
  std::unordered_map<llvm::Value*, std::string> SSAMap;
...
public:
  ...
  void EmitBranchThroughCleanupImpl(JumpDest Dest, llvm::BranchInst *BI);

    /// isObviouslyBranchWithoutCleanups - Return true if a branch to the
    /// specified destination obviously has no cleanups to run.  'false' is
    /// always a conservatively correct answer for this method.
  ...
  private:
    ...
    /// Used to create unique names for artificial VLA size debug info
    /// variables.
    ...
    /// SizeArguments - If a ParmVarDecl had the pass_object_size attribute,
    /// this will contain a mapping from said ParmVarDecl to its implicit
    /// "object_size" parameter.
    ...
    public:
      ...
      /// Emits exit block with special codegen procedure specific for the
      /// related OpenMP construct + emits code for normal construct cleanup.
      ...
      /// false otherwise.
      void enter(CodeGenFunction &CGF, OpenMPDirectiveKind Kind,
                 bool HasCancel) {
        Stack.push_back(
            {Kind,
    ...
    /// Bitmap used by MC/DC to track condition outcomes of a boolean
    /// expression.
  ...
  public:
    ...
    /// most recently incremented counter.
  ...
  private:
    ...
    /// Count the number of simple (constant) return expressions in the
    /// function.
    unsigned NumSimpleReturnExprs = 0;

    /// The last regular (non-return) debug location (breakpoint) in the
    /// function.
  ...
  public:
    ...
    /// The scope of a CXXDefaultInitExpr. Within this scope, the value of
    /// 'this' is overridden to be the object under construction.
    ...
    // Helper class for the OpenMP IR Builder. Allows reusability of code used
    // for region body, and finalization codegen callbacks. This will class will
    // also contain privatization functions used by the privatization call backs
    //
    ...
    // OMPBuilder is done, everything here will either become part of
    // CodeGenFunc. directly, or a new helper class that will contain functions
    // used by both this and the OMPBuilder
    ...
    struct OMPBuilderCBHelpers {
      ...
      /// RAII for preserving necessary info during Outlined region body
      /// codegen.
      ...
      public:
        InlinedRegionBodyRAII(CodeGenFunction &cgf, InsertPointTy &AllocaIP,
          ...
          // containing function so it expects an empty AllocaIP in which case
          // will reuse the old alloca insertion point, or a new AllocaIP in the
          // same block as the old one
  ...
  private:
    ...
    /// the constructor, but could be overwritten to true if this is a
    /// coroutine.
  ...
  public:
    ...
    void pushIrregularPartialArrayCleanup(
        llvm::Value * arrayBegin, Address arrayEndPointer, QualType elementType,
        CharUnits elementAlignment, Destroyer * destroyer);
    void pushRegularPartialArrayCleanup(
        llvm::Value * arrayBegin, llvm::Value * arrayEnd, QualType elementType,
        CharUnits elementAlignment, Destroyer * destroyer);
    ...
    llvm::Function *GenerateBlockFunction(
        GlobalDecl GD, const CGBlockInfo &Info, const DeclMapTy &ldm,
    ...
    ///
    /// \param Addr When \p LoadBlockVarAddr is false, the address of the
    /// __block structure that will be passed to _Block_object_dispose. When
    ...
    void enterByrefCleanup(CleanupKind Kind, Address Addr,
                           BlockFieldFlags Flags, bool LoadBlockVarAddr,
                           bool CanThrow);
    ...
    void EmitForwardingCallToLambda(
        const CXXMethodDecl *LambdaCallOperator, CallArgList &CallArgs,
    ...
    void getVTablePointers(
        ...
        CharUnits OffsetFromNearestVBase, bool BaseIsNonVirtualPrimaryBase,
    ...
    llvm::Value *GetVTablePtr(
        Address This, llvm::Type * VTableTy, const CXXRecordDecl *VTableClass,
    ...
    /// EmitVTablePtrCheckForCall - Virtual method MD is being called via
    /// VTable. If vptr CFI is enabled, emit a check that VTable is valid.
    void EmitVTablePtrCheckForCall(const CXXRecordDecl *RD, llvm::Value *VTable,
                                   CFITypeCheckKind TCK, SourceLocation Loc);

    /// EmitVTablePtrCheck - Emit a check that VTable is a valid virtual table
    /// for RD using llvm.type.test.
    void EmitVTablePtrCheck(const CXXRecordDecl *RD, llvm::Value *VTable,
                            CFITypeCheckKind TCK, SourceLocation Loc);

    /// If whole-program virtual table optimization is enabled, emit an
    /// assumption that VTable is a member of RD's type identifier. Or, if vptr
    /// CFI is enabled, emit a check that VTable is a member of RD's type
    /// identifier.
    ...
    /// AlwaysEmitXRayTypedEvents - Return true if clang must unconditionally
    /// emit XRay typed event handling calls.
    ...
    /// arguments for the given function. This is also responsible for naming
    /// the LLVM function arguments.
    ...
    /// branches to the given block and does not expect to emit code into it.
    /// This means the block can be ignored if it is unreachable.
    ...
    /// insertion point.
    ///
    /// This function clears the current insertion point. The caller should
    /// follow calls to this function with calls to Emit*Block prior to
    /// generation new code.
    ...
    Address mergeAddressesInConditionalExpr(
    ...
    /// authentication information needed to authenticate the signed pointer.
    Address makeNaturalAddressForPointer(
      ...
      return Address(Ptr, ConvertTypeForMem(T), Alignment,
                     CGM.getPointerAuthInfoForPointeeType(T),
    ...
    /// Same as MakeNaturalAlignPointeeAddrLValue except that the pointer is
  ...
  public:
    ...
    RawAddress CreateTempAlloca(
        llvm::Type * Ty, CharUnits align, const Twine &Name = "tmp",
        llvm::Value *ArraySize = nullptr, RawAddress *Alloca = nullptr);
    ...
    /// appropriate alignment. This routine should only be used when an
    ...
    /// EvaluateExprAsBool - Perform the usual unary conversions on the
    /// specified expression and compare the result against zero, returning an
    /// Int1Ty value.
    ...
    llvm::Value *EmitWithOriginalRHSBitfieldAssignment(
        const BinaryOperator *E, llvm::Value **Previous, QualType *SrcType);

    /// Emit a check that an [implicit] conversion of a bitfield. It is not UB,
    /// so we use the value after conversion.
    void EmitBitfieldConversionCheck(
        llvm::Value * Src, QualType SrcType, llvm::Value * Dst,
        QualType DstType, const CGBitFieldInfo &Info, SourceLocation Loc);

    /// EmitIgnoredExpr - Emit an expression in a context which ignores the
    /// result.
    void EmitIgnoredExpr(const Expr *E);

    /// EmitAnyExpr - Emit code to compute the specified expression which can
    /// have any type.  The result is returned as an RValue struct.  If this is
    /// an aggregate expression, the aggloc/agglocvolatile arguments indicate
    ...
    /// Given an opaque value expression, return its LValue mapping if it
    /// exists, otherwise create one.
    LValue getOrCreateOpaqueLValueMapping(const OpaqueValueExpr *e);

    /// Given an opaque value expression, return its RValue mapping if it
    /// exists, otherwise create one.
    ...
    /// to null, If the type contains data member pointers, they will be
    /// initialized to -1 in accordance with the Itanium C++ ABI.
    ...
    /// LoadCXXThis - Load the value of 'this'. This function is only valid
    ...
    /// LoadCXXVTT - Load the VTT parameter to base constructors/destructors
    /// have virtual bases.
    ...
    Address GetAddressOfDirectBaseInCompleteClass(
        Address Value, const CXXRecordDecl *Derived, const CXXRecordDecl *Base,
        bool BaseIsVirtual);

    static bool ShouldNullCheckClassCastValue(const CastExpr *Cast);

    /// GetAddressOfBaseClass - This function will add the necessary delta to
    /// the load of 'this' and returns address of the base class.
    ...
    /// This is necessary if the ABI does not support forwarding the arguments
    /// to the base class constructor (because they're variadic or similar).
    ...
    void EmitCXXAggrConstructorCall(
        const CXXConstructorDecl *D, const ArrayType *ArrayTy, Address ArrayPtr,
        const CXXConstructExpr *E, bool NewPointerIsChecked,
        bool ZeroInitialization = false);

    void EmitCXXAggrConstructorCall(
        const CXXConstructorDecl *D, llvm::Value *NumElements, Address ArrayPtr,
        const CXXConstructExpr *E, bool NewPointerIsChecked,
        bool ZeroInitialization = false);
    ...
    enum TypeCheckKind {
      ...
      TCK_Load,
      /// Checking the destination of a store. Must be suitably sized and
      /// aligned.
      TCK_Store,
      /// Checking the bound value in a reference binding. Must be suitably
      /// sized
      ...
      TCK_ConstructorCall,
      /// Checking the operand of a static_cast to a derived pointer type. Must
      /// be
      ...
      /// Checking the operand of a cast to a base object. Must be suitably
      /// sized
      /// and aligned.
      TCK_Upcast,
      ...
      TCK_NonnullAssign,
      /// Checking the operand of a dynamic_cast or a typeid expression.  Must
      /// be
      /// null or an object within its lifetime.
    ...
    const FieldDecl *FindFlexibleArrayMemberFieldAndOffset(
    ...
    llvm::Value *EmitCountedByFieldExpr(
    ...
    /// dimension of an array. It registers the association of its
    /// (1-dimensional) QualTypes and size expression's debug node, so that
    /// CGDebugInfo can reference this node when creating the DISubrange object
    /// to describe the array types.
    ...
    void emitAlignmentAssumption(
        llvm::Value * PtrValue, QualType Ty, SourceLocation Loc,
        SourceLocation AssumptionLoc, llvm::Value * Alignment,
        llvm::Value *OffsetValue = nullptr);

    void emitAlignmentAssumption(
        llvm::Value * PtrValue, const Expr *E, SourceLocation AssumptionLoc,
        llvm::Value *Alignment, llvm::Value *OffsetValue = nullptr);
    ...
    /// first calling EmitBlock, EmitBranch, or EmitStmt.
    ...
    Address EmitCompoundStmtWithoutScope(
        const CompoundStmt &S, bool GetLast = false,
        AggValueSlot AVS = AggValueSlot::ignored());
    ...
    void EmitCapturedLocals(CodeGenFunction & ParentCGF,
    ...
    /// parent frame.
    Address recoverAddrOfEscapedLocal(
    ...
    /// \param CopyGen Copying procedure that copies value of single array
    /// element to another single array element.
    ...
    /// \param SrcVD Source variable used in \a CopyExpr (for arrays, has type
    /// of the base array element).
    /// \param Copy Actual copygin expression for copying data from \a SrcVD to
    /// \a DestVD.
    ...
    /// not also firstprivate, then the default initialization is used.
    /// Otherwise initialization of this variable is performed by
    /// EmitOMPFirstprivateClause method.
    ///
    /// \param D Directive that may have 'lastprivate' directives.
    /// \param PrivateScope Private scope for capturing lastprivate variables
    /// for proper codegen in internal captured statement.
    ///
    ...
    /// 'i1 false' otherwise. If this item is nullptr, no final check is
    /// required.
    ...
    void processInReduction(const OMPExecutableDirective &S,
                            OMPTaskDataTy &Data, CodeGenFunction &CGF,
                            const CapturedStmt *CS, OMPPrivateScope &Scope);
    ...
    void EmitOMPTeamsGenericLoopDirective(
  ...
  public:
    ...
    void EmitOpenACCComputeConstruct(const OpenACCComputeConstruct &S) {
      // TODO OpenACC: Implement this.  It is currently implemented as a
      // 'no-op', simply emitting its structured block, but in the future we
      // will implement some sort of IR.
      EmitStmt(S.getStructuredBlock());
    }

    void EmitOpenACCLoopConstruct(const OpenACCLoopConstruct &S) {
      // TODO OpenACC: Implement this.  It is currently implemented as a
      // 'no-op', simply emitting its loop, but in the future we will implement
      // some sort of IR.
      EmitStmt(S.getLoop());
    }
    ...
    /// EmitUnsupportedLValue - Emit a dummy l-value using the type of E and
    /// issue an ErrorUnsupported style diagnostic (using the provided Name).
    ...
    /// size, this method guarantees that the returned pointer type will point
    /// to an LLVM type of the same size of the lvalue's type.  If the lvalue
    /// has a variable length type, this is not possible.
    ///
  ...
  public:
    ...
    llvm::Value *EmitLoadOfScalar(
        Address Addr, bool Volatile, QualType Ty, SourceLocation Loc,
        AlignmentSource Source = AlignmentSource::Type,
    ...
    /// EmitStoreThroughBitfieldLValue - Store Src into Dst with same
    ...
    /// if the Field is a reference, this will return the address of the
    /// reference and not the address of the value stored in the reference.
    ...
    void checkTargetFeatures(SourceLocation Loc,
    ...
    SmallVector<llvm::OperandBundleDef, 1> getBundlesForFunclet(llvm::Value *
                                                                Callee);
    ...
    CGCallee BuildAppleKextVirtualCall(
        const CXXMethodDecl *MD, NestedNameSpecifier *Qual, llvm::Type *Ty);

    CGCallee BuildAppleKextVirtualDestructorCall(
    ...
    CGPointerAuthInfo EmitPointerAuthInfo(
    ...
    llvm::Value *emitPointerAuthResign(
        llvm::Value * Pointer, QualType PointerType,
    ...
    llvm::Value *authPointerToPointerCast(
        llvm::Value * ResultPtr, QualType SourceType, QualType DestType);
    ...
    // removed.
    static std::string getNonTrivialCopyConstructorStr(
    ...
    RValue EmitCXXMemberOrOperatorMemberCallExpr(
        const CallExpr *CE, const CXXMethodDecl *MD,
    ...
    /// EmitTargetBuiltinExpr - Emit the given builtin call. Returns 0 if the
    /// call is unhandled by the current target.
    llvm::Value *EmitTargetBuiltinExpr(unsigned BuiltinID, const CallExpr *E,
                                       ReturnValueSlot ReturnValue);

    llvm::Value *EmitAArch64CompareBuiltinExpr(
        llvm::Value * Op, llvm::Type * Ty, const llvm::CmpInst::Predicate Fp,
        const llvm::CmpInst::Predicate Ip, const llvm::Twine &Name = "");
    ...
    llvm::Function *LookupNeonLLVMIntrinsic(
        unsigned IntrinsicID, unsigned Modifier, llvm::Type *ArgTy,
        const CallExpr *E);
    llvm::Value *EmitNeonCall(
        llvm::Function * F, SmallVectorImpl<llvm::Value *> & O,
        const char *name, unsigned shift = 0, bool rightshift = false);
    ...
    llvm::Value *EmitAArch64SVEBuiltinExpr(unsigned BuiltinID,
                                           const CallExpr *E);
    ...
    llvm::Value *EmitAArch64SMEBuiltinExpr(unsigned BuiltinID,
                                           const CallExpr *E);
    ...
    llvm::Value *EmitScalarOrConstFoldImmArg(unsigned ICEArguments,
    ...
    llvm::Value *EmitObjCAutorelease(llvm::Value * value,
                                     llvm::Type * returnType);
    llvm::Value *EmitObjCRetainNonBlock(llvm::Value * value,
                                        llvm::Type * returnType);
    void EmitObjCRelease(llvm::Value * value, ARCPreciseLifetime_t precise);

    std::pair<LValue, llvm::Value *> EmitARCStoreAutoreleasing(
        const BinaryOperator *e);
    std::pair<LValue, llvm::Value *> EmitARCStoreStrong(const BinaryOperator *e,
                                                        bool ignored);
    std::pair<LValue, llvm::Value *> EmitARCStoreUnsafeUnretained(
        const BinaryOperator *e, bool ignored);

    llvm::Value *EmitObjCAlloc(llvm::Value * value, llvm::Type * returnType);
    llvm::Value *EmitObjCAllocWithZone(llvm::Value * value,
                                       llvm::Type * returnType);
    llvm::Value *EmitObjCAllocInit(llvm::Value * value,
    ...
    llvm::Value *EmitComplexToScalarConversion(
        ComplexPairTy Src, QualType SrcTy, QualType DstTy, SourceLocation Loc);
    ...
    /// EmitAggExprToLValue - Emit the computation of the specified expression
    /// of aggregate type into a temporary LValue.
    ...
    /// destination address.
    void EmitAggFinalDestCopy(QualType Type, AggValueSlot Dest,
                              const LValue &Src, ExprValueKind SrcKind);

    /// Create a store to \arg DstPtr from \arg Src, truncating the stored value
    /// to at most \arg DstSize bytes.
    void CreateCoercedStore(llvm::Value * Src, Address Dst,
    ...
    ComplexPairTy EmitPromotedComplexExpr(const Expr *E,
                                          QualType PromotionType);
    llvm::Value *EmitPromotedScalarExpr(const Expr *E, QualType PromotionType);
    ComplexPairTy EmitPromotedValue(ComplexPairTy result,
                                    QualType PromotionType);
    ...
    /// initializer has a different type than GV does, this may free GV and
    /// return a different one.  Otherwise it just returns GV.
    llvm::GlobalVariable *AddInitializerToStaticVarDecl(
    ...
    llvm::Constant *createAtExitStub(
        const VarDecl &VD, llvm::FunctionCallee Dtor, llvm::Constant *Addr);
    ...
    /// Emit a branch to select whether or not to perform guarded
    /// initialization.
    void EmitCXXGuardedInitBranch(
        llvm::Value * NeedsInit, llvm::BasicBlock * InitBlock,
        llvm::BasicBlock * NoInitBlock, GuardKind Kind, const VarDecl *D);
    ...
    void EmitBranchToCounterBlock(
        const Expr *Cond, BinaryOperator::Opcode LOp,
        llvm::BasicBlock *TrueBlock, llvm::BasicBlock *FalseBlock,
        uint64_t TrueCount = 0, Stmt::Likelihood LH = Stmt::LH_None,
        const Expr *CntrIdx = nullptr);

    /// EmitBranchOnBoolExpr - Emit a branch on a boolean condition (e.g. for an
    /// if statement) to the specified blocks.  Based on the condition, this
    /// might try to simplify the codegen of the conditional based on the
    /// branch. TrueCount should be the number of times we expect the condition
    /// to evaluate to true based on PGO data.
    ...
    void EmitNullabilityCheck(LValue LHS, llvm::Value * RHS,
                              SourceLocation Loc);
    ...
    llvm::Value *EmitCheckedInBoundsGEP(
        llvm::Type * ElemTy, llvm::Value * Ptr, ArrayRef<llvm::Value *> IdxList,
        bool SignedIndices, bool IsSubtraction, SourceLocation Loc,
        const Twine &Name = "");

    Address EmitCheckedInBoundsGEP(
        Address Addr, ArrayRef<llvm::Value *> IdxList, llvm::Type * elementType,
        bool SignedIndices, bool IsSubtraction, SourceLocation Loc,
  ...
  private:
    ...
    // |ParentToken| as it's parent convergence instr.
  ...
  private:
    ...
    void deferPlaceholderReplacement(llvm::Instruction * Old,
                                     llvm::Value * New);
    ...
    /// \arg Ty, into individual arguments on the provided vector \arg
    /// IRCallArgs, starting at index \arg IRCallArgPos. See ABIArgInfo::Expand.
    void ExpandTypeToArgs(
        QualType Ty, CallArg Arg, llvm::FunctionType * IRFuncTy,
        SmallVectorImpl<llvm::Value *> & IRCallArgs, unsigned &IRCallArgPos);

    std::pair<llvm::Value *, llvm::Type *> EmitAsmInput(
        const TargetInfo::ConstraintInfo &Info, const Expr *InputExpr,
        std::string &ConstraintStr);

    std::pair<llvm::Value *, llvm::Type *> EmitAsmInputLValue(
    ...
    /// function is aware of pass_object_size parameters, and will act
    /// accordingly if E is a parameter with the pass_object_size attribute.
  ...
  public:
    ...
    /// explicit source.
    Address EmitPointerWithAlignment(
        const Expr *Addr, LValueBaseInfo *BaseInfo = nullptr,
        TBAAAccessInfo *TBAAInfo = nullptr,
        KnownNonNull_t IsKnownNonNull = NotKnownNonNull);

    /// If \p E references a parameter with pass_object_size info or a constant
    /// array size modifier, emit the object size divided by the size of \p
    /// EltTy. Otherwise return null.
    ...
    void EmitX86MultiVersionResolver(
        llvm::Function * Resolver,
        ArrayRef<MultiVersionResolverOption> Options);
    void EmitAArch64MultiVersionResolver(
        llvm::Function * Resolver,
        ArrayRef<MultiVersionResolverOption> Options);
  ...
  private:
    ...
    llvm::Value *FormAArch64ResolverCondition(
