...
llvm::Value *CodeGenFunction::EmitLoadOfScalar(Address Addr, bool Volatile,
  ...
  // std::string OriginalSource = "unknown";

  // // Check if the pointer has a variable name
  // if (Ptr->hasName()) {
  //     VarName = Ptr->getName().str();
  // }

  // // 🔹 Ensure original variable tracking
  // if (OriginalVariable.find(VarName) == OriginalVariable.end()) {
  //     OriginalVariable[VarName] = VarName;
  // }

  // // 🔹 Check if we are loading a previously stored variable
  // if (SSAMap.find(Ptr) != SSAMap.end()) {
  //     OriginalSource = SSAMap[Ptr];  // Retrieve the last known source of the
  //     variable
  // } else {
  //     OriginalSource = VarName;
  // }

  // // 🔹 Construct detailed load metadata
  // std::string LoadInfo = VarName + " = " + OriginalSource;

  // // 🔹 Attach metadata
  // llvm::MDNode *LoadMetadata = llvm::MDNode::get(
  //     Load->getContext(), llvm::MDString::get(Load->getContext(), LoadInfo));
  // Load->setMetadata("load_info", LoadMetadata);

  // // 🔹 Update SSA Map to reflect correct mapping
  // SSAMap[Load] = OriginalSource;
  // 🔹 Ensure the Load instruction has a unique name
  // 🔹 Ensure the Load instruction has a unique name
  std::string LoadName;
  if (Load->hasName()) {
    LoadName = Load->getName().str();
  } else {
    LoadName = "tmp" + std::to_string(SSAMap.size());
    Load->setName(LoadName); // Assign name to unnamed loads
  }
  llvm::Type *LoadedType = Load->getType();
  bool IsPointer = LoadedType->isPointerTy();

  std::string VarName = "unknown";
  // 🔹 Get the name of the loaded variable (pointer)
  if (Ptr->hasName()) {
    VarName = Ptr->getName().str();
  }
  ...
  // 🔹 Resolve SSA mapping correctly (avoid infinite loops)
  std::set<std::string> Visited;
  while (VarName.rfind("tmp", 0) == 0 && SSAMap.find(Ptr) != SSAMap.end()) {
    if (Visited.count(VarName))
      break; // Detect circular references
    Visited.insert(VarName);
    VarName = SSAMap[Ptr];
  }

  if (IsPointer) {
    // Handle pointer loads correctly
    if (SSAMap.find(Ptr) != SSAMap.end()) {
      VarName = SSAMap[Ptr]; // Use mapped value if available
    } else {
      VarName = "*" + VarName; // Only apply `*` to actual pointers
    }
  } else {
    // Handle integer variables properly
    if (SSAMap.find(Ptr) != SSAMap.end()) {
      VarName = SSAMap[Ptr]; // Use mapped SSA value
    }
    // ❌ Do NOT add `*` for integers!
  }
  ...
  std::string LoadInfo = LoadName + " = " + VarName;
  llvm::MDNode *LoadMetadata = llvm::MDNode::get(
      Load->getContext(), llvm::MDString::get(Load->getContext(), LoadInfo));
  Load->setMetadata("load_info", LoadMetadata);

  llvm::errs() << "Updated SSAMap: [" << Load << "] (" << LoadName
               << ") = " << VarName << "\n";
...
void CodeGenFunction::EmitStoreOfScalar(llvm::Value *Value, Address Addr,
  ...
  if (llvm::ConstantInt *CInt = llvm::dyn_cast<llvm::ConstantInt>(Value)) {
    ...
    CInt->getValue().toString(Str, 10, true, false, false, false);
    std::string ConstValue(Str.begin(), Str.end());
    SSAMap[Value] = ConstValue;
  }

  llvm::StoreInst *Store = Builder.CreateStore(Value, Addr, Volatile);

  // llvm::Value *StoredValue = Value;
  // llvm::Value *Ptr = Addr.getBasePointer();
  // std::string VarName = "unknown";
  // std::string FuncName = "";
  // std::string params = "";

  // // 🔹 Check if we are storing a previously loaded value
  // std::string OriginalSource = "unknown";
  // if (SSAMap.find(StoredValue) != SSAMap.end()) {
  //   OriginalSource = SSAMap[StoredValue]; // Retrieve original variable
  // } else if (StoredValue->hasName()) {
  //   OriginalSource = StoredValue->getName().str();
  // }

  // // 🔹 Ensure variable tracking
  // if (Ptr->hasName()) {
  //   VarName = Ptr->getName().str();
  // }
  ...
  // if (llvm::Instruction *Instr =
  //         llvm::dyn_cast<llvm::Instruction>(StoredValue)) {
  //   if (Instr->isBinaryOp()) {
  //     std::string LHS = "unknown", RHS = "unknown";

  //     // Get operand names if available
  //     if (SSAMap.find(Instr->getOperand(0)) != SSAMap.end())
  //       LHS = SSAMap[Instr->getOperand(0)];
  //     else if (Instr->getOperand(0)->hasName())
  //       LHS = Instr->getOperand(0)->getName().str();

  //     if (SSAMap.find(Instr->getOperand(1)) != SSAMap.end())
  //       RHS = SSAMap[Instr->getOperand(1)];
  //     else if (Instr->getOperand(1)->hasName())
  //       RHS = Instr->getOperand(1)->getName().str();

  //     // Assign a name to this computed value
  //     std::string OpType = "unknown";

  //     // Detect the operator dynamically
  //     if (Instr->getOpcode() == llvm::Instruction::Add)
  //       OpType = "+";
  //     else if (Instr->getOpcode() == llvm::Instruction::Sub)
  //       OpType = "-";
  //     else if (Instr->getOpcode() == llvm::Instruction::Mul)
  //       OpType = "*";
  //     else if (Instr->getOpcode() == llvm::Instruction::SDiv)
  //       OpType = "/";
  //     else if (Instr->getOpcode() == llvm::Instruction::SRem)
  //       OpType = "%";

  //     // Construct the metadata for the operation
  //     std::string ComputedName = LHS + " " + OpType + " " + RHS;

  //     // Increment the SSA version for the new result based on the LHS
  //     SSAVersions[ComputedName] = ++SSAVersions[LHS];

  //     // **Update the original source to the computed value**
  //     OriginalSource = ComputedName;

  //     // Construct the final metadata string
  //     std::string ArithMetadata =
  //         VarName + " = " + LHS + " " + OpType + " " + RHS; // change this
  //         line

  //     // 🔹 Attach metadata to the store instruction
  //     llvm::MDNode *ArithMD = llvm::MDNode::get(
  //         Builder.getContext(),
  //         llvm::MDString::get(Builder.getContext(), ArithMetadata));
  //     if (llvm::Instruction *Inst =
  //             llvm::dyn_cast<llvm::Instruction>(StoredValue)) {
  //       Inst->setMetadata("arith_info",
  //                         ArithMD); // Set metadata on the instruction
  //     }
  //   }

  //   if (llvm::CallInst *Call = llvm::dyn_cast<llvm::CallInst>(StoredValue)) {
  //     // Get the function name
  //     FuncName = Call->getCalledFunction()->getName().str();

  //     // Create a string to hold the parameters (do not include the function
  //     // name here)
  //     params = "(";
  //     bool first = true;
  ...
  //       std::string ArgName = "unknown";

  //       // Check if argument has a name or SSA mapping
  //       if (SSAMap.find(Arg) != SSAMap.end()) {
  //         if (!first) {
  //           params += ", ";
  //         }
  //         ArgName = SSAMap[Arg];
  ...
  //     // Construct the metadata for the function call, including parameters
  //     (do
  //     // not add FuncName again)
  //     std::string CallMetadata = VarName + " = " + FuncName + params;

  //     // Attach the function call metadata to the store instruction
  //     llvm::MDNode *CallMD = llvm::MDNode::get(
  //         Builder.getContext(),
  //         llvm::MDString::get(Builder.getContext(), CallMetadata));
  //     if (llvm::Instruction *Inst =
  //             llvm::dyn_cast<llvm::Instruction>(StoredValue)) {
  //       Inst->setMetadata(
  //           "call_info",
  //           CallMD); // Attach metadata to the function call instruction
  //     }
  //   }
  // }
  // std::string StoreInfo = "";
  // // 🔹 Construct metadata for the store operation, including destination
  // (e.g.,
  // // num3)
  // if (OriginalSource == "call") {
  //   llvm::errs() << "OriginalSource: " << OriginalSource << "\n";
  //   StoreInfo = VarName + " = " + FuncName + params;
  // } else {
  //   StoreInfo = VarName + " = " + OriginalSource;
  // }
  // // 🔹 Attach metadata to the store instruction
  // llvm::MDNode *StoreMetadata = llvm::MDNode::get(
  //     Store->getContext(), llvm::MDString::get(Store->getContext(),
  //     StoreInfo));
  // Store->setMetadata("store_info", StoreMetadata);

  // // 🔹 **Update SSA Map: Track what variable was stored where**
  // SSAMap[Ptr] = VarName;
  llvm::Value *StoredValue = Value;
  llvm::Value *Ptr = Addr.getBasePointer();
  std::string VarName = "unknown";
  ...
  // 🔹 Check if the stored value has a previous SSA mapping
  std::string OriginalSource = "unknown";
  if (SSAMap.find(StoredValue) != SSAMap.end()) {
    OriginalSource = SSAMap[StoredValue]; // Track original source
    // llvm::errs() << "OriginalSource from SSAMap: " << OriginalSource << "\n";
  } else if (StoredValue->hasName()) {
    OriginalSource = StoredValue->getName().str();
    // llvm::errs() << "OriginalSource from name: " << OriginalSource << "\n";
  }
  bool isPointer = false;
  if (StoredValue->getType()->isPointerTy()) {
    isPointer = true;
    // llvm::errs() << "StoredValue is a pointer\n";
  }
  if (SSAMap.find(Ptr) != SSAMap.end()) {
    VarName = SSAMap[Ptr]; // Use the mapped SSA variable if available
    // llvm::errs() << "VarName from SSAMap: " << VarName << "\n";
  } else if (Ptr->hasName()) {
    VarName = Ptr->getName().str();
    // llvm::errs() << "VarName from name: " << VarName << "\n";
  }
  ...
  if (llvm::Instruction *Instr =
    ...
    if (Instr->isBinaryOp()) {
      ...
      // // Get original names for operands
      // if (SSAMap.find(Instr->getOperand(0)) != SSAMap.end())
      //     LHS = SSAMap[Instr->getOperand(0)];
      // else if (Instr->getOperand(0)->hasName())
      //     LHS = Instr->getOperand(0)->getName().str();
      ...
      // Get operand 0
      if (auto *CI = dyn_cast<llvm::ConstantInt>(Instr->getOperand(0))) {
        std::string strValue;
        llvm::raw_string_ostream rso(strValue);
        CI->getValue().print(rso, true); // 'true' = isSigned
        rso.flush();
        LHS = strValue;
      } else if (SSAMap.count(Instr->getOperand(0))) {
        LHS = SSAMap[Instr->getOperand(0)];
      } else if (Instr->getOperand(0)->hasName()) {
        LHS = Instr->getOperand(0)->getName().str();
      }
      ...
      // Get operand 1
      if (auto *CI = dyn_cast<llvm::ConstantInt>(Instr->getOperand(1))) {
        std::string strValue;
        llvm::raw_string_ostream rso(strValue);
        CI->getValue().print(rso, true); // 'true' = isSigned
        rso.flush();
        RHS = strValue;
      } else if (SSAMap.count(Instr->getOperand(1))) {
        RHS = SSAMap[Instr->getOperand(1)];
      } else if (Instr->getOperand(1)->hasName()) {
        RHS = Instr->getOperand(1)->getName().str();
      }
      ...
      // Construct final arithmetic tracking
      std::string ComputedName = LHS + " " + OpType + " " + RHS;
      SSAVersions[ComputedName] = ++SSAVersions[LHS];
      OriginalSource = ComputedName;
      // SSAMap[Instr] = ComputedName;
      // llvm::errs() << "Updated SSAMap: [" << Instr  << "] = " << ComputedName
      // << "\n"; Attach metadata if (IsPointer) {
      //   OriginalSource = "&" + OriginalSource;
      // }
      std::string ArithMetadata = VarName + " = " + ComputedName;
      llvm::MDNode *ArithMD = llvm::MDNode::get(
          Builder.getContext(),
          llvm::MDString::get(Builder.getContext(), ArithMetadata));
      Instr->setMetadata("arith_info", ArithMD);
    }
    ...
    if (llvm::CallInst *Call = llvm::dyn_cast<llvm::CallInst>(StoredValue)) {
      ...
      llvm::MDNode *CallMD = llvm::MDNode::get(
          Builder.getContext(),
          llvm::MDString::get(Builder.getContext(), CallMetadata));
      Call->setMetadata("call_info", CallMD);

      OriginalSource = FuncName + params; // Track function call
    }
  }
  ...
  std::string StoreInfo = VarName + " = " + OriginalSource;
  // llvm::errs() << "StoreInfo: " << StoreInfo << "\n";
  llvm::MDNode *StoreMetadata = llvm::MDNode::get(
      Store->getContext(), llvm::MDString::get(Store->getContext(), StoreInfo));
  Store->setMetadata("store_info", StoreMetadata);

  // 🔹 **Update SSA Map: Track the true source of the stored value**
  // if (SSAMap.find(StoredValue) != SSAMap.end()) {
  //     SSAMap[Ptr] = SSAMap[StoredValue];  // Track the original source
  // } else {
  //     SSAMap[Ptr] = OriginalSource;
  // }
