...
// Get function return type (including storage specifiers)
...
for (const VarDecl *VD : Args) {
    if (const ParmVarDecl *PVD = dyn_cast<ParmVarDecl>(VD)) {
        std::string ParamType = PVD->getType().getAsString();
        std::string ParamName = PVD->getNameAsString();

        if (!first)
            FuncParams += ", ";  // Separate parameters with ", "
        FuncParams += ParamType + " " + ParamName;
        first = false;
    }
}

// **Format the metadata as required**
std::string FuncMetadataStr = "{" + ReturnType + "; " + FuncParams + "}";

// Store as LLVM Metadata
llvm::MDNode *FuncMetadata = llvm::MDNode::get(
    Ctx, llvm::MDString::get(Ctx, FuncMetadataStr));

Fn->setMetadata("func_signature", FuncMetadata);
