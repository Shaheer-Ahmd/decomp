...
enum MisleadingStatementKind { MSK_if, MSK_else, MSK_for,MSK_else_if, MSK_while };
...
StmtResult Parser::ParseIfStatement(SourceLocation *TrailingElseLoc) {
  ...
  ElseLoc = ConsumeToken(); // consume 'else'
  ElseStmtLoc = Tok.getLocation();

  bool IsElseIf = Tok.is(tok::kw_if); // <-- Check if it's an 'else if'
  ...
  MisleadingIndentationChecker MIChecker(*this,
                                        IsElseIf ? MSK_else_if : MSK_else,
                                        ElseLoc);
  ...
  // ✅ Mark the nested IfStmt as being part of an else-if chain
  if (IsElseIf && ElseStmt.isUsable()) {
    if (auto *InnerIf = dyn_cast_or_null<IfStmt>(ElseStmt.get())) {
      InnerIf->setIsElseIf(true);
    }
  }
