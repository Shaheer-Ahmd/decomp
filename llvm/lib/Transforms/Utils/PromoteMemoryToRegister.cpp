...
// Extract first MDString operand if present (safe)
static std::string getFirstMDString(MDNode *N) {
...
static std::string valueToString(const Value *V) {
  ...
  // Print as an operand, without types ("%x" or constant form).
...
struct PromoteMem2Reg {
  ...
  /// Per-store metadata id counter used to generate "store_id:N".
  unsigned StoreMetaID = 0;

  unsigned LoadMetaID = 0;
...
static void emitAllocaMetadata(const std::vector<AllocaInst *> &Allocas) {
  if (Allocas.empty())
    return;
  Module *M = Allocas[0]->getModule();
  LLVMContext &Ctx = M->getContext();
  NamedMDNode *NMD = M->getOrInsertNamedMetadata("alloca_infos");
  unsigned StoreMetaID = 1;
  for (AllocaInst *AI : Allocas) {
    if (!AI)
      continue;
    if (MDNode *SSAMD = AI->getMetadata("ssa_info")) {
      if (SSAMD->getNumOperands() > 0) {
        if (MDString *S = dyn_cast<MDString>(SSAMD->getOperand(0))) {
          std::string s = S->getString().str();
          // s is of the form "<varname> : <type> : SSA_0"
          ...
          if (firstColon != std::string::npos &&
            ...
            MFields.push_back(MDString::get(Ctx, "alloca_id:" + std::to_string(StoreMetaID++)));
            MFields.push_back(MDString::get(Ctx, "name:" + varName));
            MFields.push_back(MDString::get(Ctx, "type:" + type));
            MFields.push_back(MDString::get(Ctx, "block:" + BB->getName().str()));
            NMD->addOperand(MDNode::get(Ctx, MFields));
          }
        }
      }
    }
...
static void emitStoreMetadata(StoreInst *SI, BasicBlock *BB,
                              unsigned &StoreMetaID,
                              DenseMap<BasicBlock *, unsigned> &BBNumbers) {
  Module *M = BB->getParent()->getParent();
  LLVMContext &Ctx = M->getContext();
  if (SI->getMetadata("store_info") || SI->getDebugLoc().get() != nullptr) {
    unsigned ID = ++StoreMetaID;
    std::string sid = "store_id:" + std::to_string(ID);
    BasicBlock *StoreBB = SI->getParent();
    std::string blockName;
    if (StoreBB->hasName())
      blockName = StoreBB->getName().str();
    else {
      auto It = BBNumbers.find(StoreBB);
      if (It != BBNumbers.end())
        blockName = std::string("bb") + std::to_string(It->second);
      else
        blockName = "<unnamed_bb>";
    }
    std::string blockField = "block:" + BB->getName().str();
    Value *Ptr = SI->getPointerOperand();
    std::string ptrName = Ptr->hasName() ? Ptr->getName().str() : "<ptr>";
    std::string assign;
    if (MDNode *StoreMD = SI->getMetadata("store_info")) {
      std::string info = getFirstMDString(StoreMD);
      info = trimString(info);
      if (!info.empty()) {
        size_t eq = info.find('=');
        if (eq != std::string::npos) {
          std::string lhs = trimString(info.substr(0, eq));
          std::string rhs = trimString(info.substr(eq + 1));
    ...
    if (assign.empty()) {
      std::string ptrNameLocal =
          Ptr->hasName() ? Ptr->getName().str() : "<ptr>";
      if (!ptrNameLocal.empty() &&
          (ptrNameLocal.front() == '%' || ptrNameLocal.front() == '@'))
        ptrNameLocal.erase(ptrNameLocal.begin());
      std::string valStr = valueToString(SI->getValueOperand());
      if (!valStr.empty() && (valStr.front() == '%' || valStr.front() == '@'))
        valStr.erase(valStr.begin());
      assign = "assign:" + ptrNameLocal + "=" + valStr;
    }
    SmallVector<Metadata *, 3> MFields;
    MFields.push_back(MDString::get(Ctx, sid));
    MFields.push_back(MDString::get(Ctx, blockField));
    MFields.push_back(MDString::get(Ctx, assign));
    NamedMDNode *NMD = M->getOrInsertNamedMetadata("store_infos");
    NMD->addOperand(MDNode::get(Ctx, MFields));
  }
}
...
NextIteration:
  errs() << "RenamePass Iteration for: " << BB->getName() << "\n";
  ...
  for (BasicBlock::iterator II = BB->begin(); !II->isTerminator();) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
      ...
      // --- Attach per-user metadata saying "this use was from var X" (fixed)
      // ---

      LLVMContext &Ctx = LI->getContext();

      // --- prefer load_info metadata for variable name / info ---
      std::string varName = "<ptr>";

      // Try load_info metadata first (e.g. !load_info !17  where !17 =
      // !{"tmp2 = x"})
      if (MDNode *LoadMD = LI->getMetadata("load_info")) {
        std::string info = getFirstMDString(LoadMD); // e.g. "tmp2 = x"
        info = trimString(info);
        if (!info.empty()) {
          // Try to extract RHS after '=' if present, else use whole info
          // string
          size_t eq = info.find('=');
          if (eq != std::string::npos) {
            std::string rhs = trimString(info.substr(eq + 1));
            ...
            varName = info; // fallback to "tmp2 = x" or whatever MD gave
          }
        }
      }

      // If still default, fall back to old behavior (alloca/pointer name or
      // debug loc)
      if (varName == "<ptr>") {
        if (Src && Src->hasName()) {
          varName = Src->getName().str();
        } else if (LI->getPointerOperand()->hasName()) {
          varName = LI->getPointerOperand()->getName().str();
        } else if (LI->getDebugLoc().get() != nullptr) {
          DebugLoc DL = LI->getDebugLoc();
          if (MDNode *ScopeMD = DL.getScope()) {
            if (auto *Scope = dyn_cast<DIScope>(ScopeMD)) {
              unsigned Line = DL.getLine();
              std::string FN = Scope->getFilename().str();
              varName = FN + ":" + std::to_string(Line);
            }
          }
        }
      }
      ...
      if (!varName.empty() &&
          (varName.front() == '%' || varName.front() == '@'))
        varName.erase(varName.begin());

      SmallVector<std::pair<Instruction *, unsigned>, 8> UsersToAnnotate;
      for (Use &U : make_early_inc_range(LI->uses())) {
        Instruction *UserI = cast<Instruction>(U.getUser());
        for (unsigned oi = 0, e = UserI->getNumOperands(); oi != e; ++oi) {
          if (UserI->getOperand(oi) == LI)
            UsersToAnnotate.emplace_back(UserI, oi);
        }
      }

      // Attach metadata to each user instruction (merge & dedupe existing
      // entries)
      for (auto &P : UsersToAnnotate) {
        Instruction *UserI = P.first;
        unsigned OpIdx = P.second;

        // Build entry like "0:x" (operandIndex:varName)
        std::string entry = std::to_string(OpIdx) + ":" + varName;

        // Gather existing strings (if any) to avoid duplicates
        SmallVector<Metadata *, 8> MFields;
        SmallSet<std::string, 4> Seen;
        if (MDNode *Old = UserI->getMetadata("orig_loads")) {
          for (unsigned i = 0, ne = Old->getNumOperands(); i != ne; ++i) {
            if (MDString *S = dyn_cast<MDString>(Old->getOperand(i))) {
              std::string s = S->getString().str();
        ...
        if (Seen.insert(entry).second)
          MFields.push_back(MDString::get(Ctx, entry));

        // Set the merged metadata back on the user instruction
        UserI->setMetadata("orig_loads", MDNode::get(Ctx, MFields));
      }

      // One convert-to-assume call only (before replacing uses).
      convertMetadataToAssumes(LI, V, SQ.DL, AC, &DT);

      // Replace uses and erase the load.
      ...
      emitStoreMetadata(SI, BB, StoreMetaID, BBNumbers);
      // Helper to emit store metadata for a store instruction
      SI->eraseFromParent();
    }
  }
