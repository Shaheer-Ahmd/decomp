...
// =============== UPDATE METADATA PATCH START (DECLARATION) ===============
  // To handle unique suffixing applied by LLVM's ValueSymbolTable (e.g., ".i16", "atoi.exit54"),
  // we build a 1-to-1 map of Original Block Names -> Newly Assigned Cloned Names.
  auto UpdateTrimmerMetadata = [&](const std::string &ReturnBBNewName) {
    const llvm::StringRef SimpleMDKeys[] = {
      llvm::mdk::IncBlockKey, llvm::mdk::ContBlockKey,
      llvm::mdk::ConditionBlockKey, llvm::mdk::BodyBlockKey,
      llvm::mdk::DefaultSuccKey, llvm::mdk::IfContBlockKey,
    };
    const llvm::StringRef TreeMDKeys[] = { llvm::mdk::CompoundConditionTree };
    const llvm::StringRef ArrayMDKeys[] = { llvm::mdk::CompoundConditionSuccessorsKey }; 
    llvm::LLVMContext &Ctx = Caller->getContext();

    // 1. Build the mapping dictionary dynamically from the newly grafted blocks
    llvm::StringMap<std::string> OrigToNewName;
    for (llvm::BasicBlock &NewBB : llvm::make_range(FirstNewBlock->getIterator(), Caller->end())) {
      llvm::StringRef NewName = NewBB.getName();
      // CloneAndPruneFunctionInto uses ".i" as the baseline suffix.
      size_t DotPos = NewName.find(".i"); 
      if (DotPos != llvm::StringRef::npos) {
        llvm::StringRef OrigName = NewName.substr(0, DotPos);
        OrigToNewName[OrigName] = NewName.str();
      } else {
        OrigToNewName[NewName] = NewName.str();
      }
    }

    // 2. Map the Return block to its true destination (e.g., "atoi.exit54")
    if (Returns.size() == 1 && !ReturnBBNewName.empty()) {
      llvm::StringRef RetBBName = Returns[0]->getParent()->getName();
      size_t DotPos = RetBBName.find(".i");
      llvm::StringRef OrigRetBBName = (DotPos != llvm::StringRef::npos) ? 
                                      RetBBName.substr(0, DotPos) : RetBBName;
      OrigToNewName[OrigRetBBName] = ReturnBBNewName;
    }

    // 3. Helper to look up the exact, collision-resolved block name
    auto GetTargetName = [&](llvm::StringRef OrigStr) -> std::string {
      auto It = OrigToNewName.find(OrigStr);
      if (It != OrigToNewName.end()) {
        return It->second; // Returns exact name, e.g., "while.cond.i16"
      }
      return OrigStr.str(); // Fallback if block wasn't part of this inline instance
    };

    // 4. Recursive lambda for Tree Keys
    auto RebuildTreeMD = [&](auto &Self, llvm::Metadata *MD) -> llvm::Metadata * {
      auto *Node = llvm::dyn_cast_or_null<llvm::MDNode>(MD);
      if (!Node || Node->getNumOperands() == 0) return MD;
      auto *Op0 = llvm::dyn_cast_or_null<llvm::MDString>(Node->getOperand(0));
      if (!Op0) return MD;

      if (Op0->getString() == "cond" && Node->getNumOperands() >= 2) {
        if (auto *BlockNameOp = llvm::dyn_cast_or_null<llvm::MDString>(Node->getOperand(1))) {
          std::string TargetStr = GetTargetName(BlockNameOp->getString());
          if (TargetStr != BlockNameOp->getString().str()) {
            llvm::SmallVector<llvm::Metadata *, 2> NewOps;
            NewOps.push_back(Op0);
            NewOps.push_back(llvm::MDString::get(Ctx, TargetStr));
            for (unsigned i = 2; i < Node->getNumOperands(); ++i) NewOps.push_back(Node->getOperand(i));
            return llvm::MDNode::get(Ctx, NewOps);
          }
        }
      } else if (Op0->getString() == "binop" && Node->getNumOperands() >= 4) {
        llvm::SmallVector<llvm::Metadata *, 4> NewOps;
        NewOps.push_back(Node->getOperand(0));
        NewOps.push_back(Node->getOperand(1));
        NewOps.push_back(Self(Self, Node->getOperand(2)));
        NewOps.push_back(Self(Self, Node->getOperand(3)));
        for (unsigned i = 4; i < Node->getNumOperands(); ++i) NewOps.push_back(Node->getOperand(i));
        return llvm::MDNode::get(Ctx, NewOps);
      }
      return MD;
    };

    // 5. Apply transformations utilizing the dictionary
    for (llvm::BasicBlock &NewBB : llvm::make_range(FirstNewBlock->getIterator(), Caller->end())) {
      for (llvm::Instruction &I : NewBB) {
        // Simple Keys
        for (const llvm::StringRef Key : SimpleMDKeys) {
          if (llvm::MDNode *MD = I.getMetadata(Key)) {
            if (MD->getNumOperands() > 0) {
              if (llvm::MDString *MDS = llvm::dyn_cast_or_null<llvm::MDString>(MD->getOperand(0))) {
                std::string TargetStr = GetTargetName(MDS->getString());
                if (TargetStr != MDS->getString().str())
                  I.setMetadata(Key, llvm::MDNode::get(Ctx, llvm::MDString::get(Ctx, TargetStr)));
              }
            }
          }
        }
        // Tree Keys
        for (const llvm::StringRef Key : TreeMDKeys) {
          if (llvm::MDNode *MD = I.getMetadata(Key)) {
            llvm::Metadata *NewTree = RebuildTreeMD(RebuildTreeMD, MD);
            if (NewTree != MD) I.setMetadata(Key, llvm::cast<llvm::MDNode>(NewTree));
          }
        }
        // Array Keys
        for (const llvm::StringRef Key : ArrayMDKeys) {
          if (llvm::MDNode *MD = I.getMetadata(Key)) {
            llvm::SmallVector<llvm::Metadata *, 4> NewOps;
            bool Changed = false;
            for (const llvm::MDOperand &Op : MD->operands()) {
              if (auto *MDS = llvm::dyn_cast_or_null<llvm::MDString>(Op.get())) {
                std::string TargetStr = GetTargetName(MDS->getString());
                if (TargetStr != MDS->getString().str()) {
                  NewOps.push_back(llvm::MDString::get(Ctx, TargetStr));
                  Changed = true;
            ...
            if (Changed) I.setMetadata(Key, MD->isDistinct() ? llvm::MDNode::getDistinct(Ctx, NewOps) : llvm::MDNode::get(Ctx, NewOps));
          }
        }
      }
    }
  };
  // =============== UPDATE METADATA PATCH END (DECLARATION) ===============
  
  // If we cloned in _exactly one_ basic block, and if that block ends in a
  // return instruction, we splice the body of the inlined callee directly into
  // the calling basic block.
  if (Returns.size() == 1 && std::distance(FirstNewBlock, Caller->end()) == 1) {
    UpdateTrimmerMetadata(OrigBB->getName().str());
  ...
  // =============== UPDATE METADATA PATCH (INVOCATION) ===============
  // AfterCallBB now has its finalized, unique symbol table name (e.g., atoi.exit54).
  // We safely map the destroyed ReturnBB metadata pointers to this new block.
  UpdateTrimmerMetadata((Returns.size() == 1) ? AfterCallBB->getName().str() : "");
  // ==================================================================
