  ...
  for (inst_iterator I = inst_begin(&F), E = inst_end(&F); I != E; ++I) {
    ...
    if (AllocaInst *AI = isDirectAlloca(&*I)) {
      ...
      if (MDNode *VarInfo =
        ...
        // std::cout << "Metadata found for variable\n";

        if (MDString *MDStr = dyn_cast<MDString>(VarInfo->getOperand(0))) {
          std::string VarTypeStr = MDStr->getString().str();

          size_t firstColon = VarTypeStr.find(":");
          size_t secondColon = VarTypeStr.find(":", firstColon + 1);

          if (firstColon != std::string::npos &&
              secondColon != std::string::npos) {
            std::string ExtractedType =
                VarTypeStr.substr(firstColon + 2, secondColon - firstColon - 3);
            // std::cout << "Extracted Type: " << ExtractedType << "\n";
            metadata_extracted = true;

            // Use extracted type for printing
            customPrintSimpleType(Out, ExtractedType);
          } else {
            // Metadata format is incorrect, fallback to default printing
            printTypeNameForAddressableValue(Out, AI->getAllocatedType(),
                                             false);
          }
        }
      } else {
        // No metadata found, fallback to default printing
        // std::cout << "Metadata not found for variable, using default type
        // printing\n";
        printTypeNameForAddressableValue(Out, AI->getAllocatedType(), false);
      }

      Out << ' ' << GetValueName(AI);
      std::cout << "printing " << GetValueName(AI) << ";\n";
...
void CWriter::printBasicBlock(BasicBlock *BB) {
  ...
  for (BasicBlock::iterator II = BB->begin(), E = --BB->end(); II != E; ++II) {
    // Your existing instruction processing logic here
...
//       CmpDeclTypes.insert(
//           std::pair<CmpInst::Predicate, VectorType *>(I.getPredicate(),
//           VTy));
...
//   // === Custom Metadata Handling Start ===
//   StringRef ReplacementOp0, ReplacementOp1;
//   if (MDNode *N = I.getMetadata("new_if_metadata")) {
//     if (MDString *S = dyn_cast<MDString>(N->getOperand(0))) {
//       StringRef MetadataStr = S->getString(); // e.g., "operand_1_tmp =
//       source1;
//                                               // operand_2_tmp = source2"

//       // Simple parsing logic for expected format
//       SmallVector<StringRef, 4> Assignments;
//       MetadataStr.split(Assignments, ';', /*MaxSplit=*/-1,
//       /*KeepEmpty=*/false); StringRef LHS, RHS; if
//       (!Assignments[0].trim().empty() && Assignments[0].contains('=')) {
//         std::tie(LHS, RHS) = Assignments[0].split('=');
//         RHS = RHS.trim();
//         ReplacementOp0 = RHS;
//       }
//       if (!Assignments[1].trim().empty() && Assignments[1].contains('=')) {
//         std::tie(LHS, RHS) = Assignments[1].split('=');
//         RHS = RHS.trim();
//         ReplacementOp1 = RHS;
//       }
//     }
//   }
//   // === Custom Metadata Handling End ===

//   bool NeedsClosingParens = writeInstructionCast(I);

//   // Write first operand
//   if (!ReplacementOp0.empty()) {
//     Out << ReplacementOp0;
//   } else {
...
void CWriter::visitIcmpInstHandleBinOp(Value *Val) {
  auto *BinOp = dyn_cast<BinaryOperator>(Val);
  if (!BinOp) {
    Out << "\nError:\n Value is not a BinaryOperator.\n";
    return;
  }

  if (auto *Load = dyn_cast<LoadInst>(BinOp->getOperand(0))) {
    if (MDNode *N = Load->getMetadata("load_info")) {
      if (MDString *S = dyn_cast<MDString>(N->getOperand(0))) {
        StringRef MetadataStr = S->getString(); // e.g., "tmp2 = num1"
        auto Parts = MetadataStr.split('=');
        StringRef SourceVar = Parts.second.trim();

        // Print (source OP operand1)
        Out << "(" << SourceVar << " ";
        ...
        case Instruction::Mul:
          ...
          break;
        default:
          // Fall back if unknown binary op
          Out << "? ";
          break;
        }

        writeOperand(BinOp->getOperand(1), ContextCasted);
        Out << ")";
      }
    } else {
      Out << "\nError:\n No load_info metadata found for BinaryOperator.\n";
    }
  } else {
    Out << "\nError:\n No load_info metadata found for BinaryOperator.\n";
  }
...
void CWriter::visitIcmpInstHandleLoad(Value *Val) {
  auto *Load = dyn_cast<LoadInst>(Val);
  if (!Load) {
    Out << "\nError:\n Value is not a LoadInst.\n";
    return;
  }
  if (MDNode *N = Load->getMetadata("load_info")) {
    if (MDString *S = dyn_cast<MDString>(N->getOperand(0))) {
    ...
    Out << "\nError:\n No load_info metadata found for LoadInst.\n";
  }
}
